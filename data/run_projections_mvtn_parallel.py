...
train_loader = CustomDataLoader(dset_train, batch_size=bs, shuffle=False, drop_last=False, pin_memory=True)
test_loader = CustomDataLoader(dset_test, batch_size=bs, shuffle=False, drop_last=False, pin_memory=True)
all_loader = list(enumerate(train_loader)) + list(enumerate(test_loader, start=len(train_loader)))
if args.end_idx != -1: loader_items = all_loader[args.start_idx:args.end_idx]
else: loader_items = all_loader
total_batches = len(all_loader)
...
print((f"📦​​​ Total dataset : {total_batches} samples\n"))
...
for i, (batch_idx, (__, __, points, names)) in enumerate(loader_items):   
    ...
    # at the end of processing each item:
    filled_len = int(bar_len * (i + args.start_idx) // total_batches)
    bar = '█' * filled_len + '-' * (bar_len - filled_len)
    percent = ((i + args.start_idx) / total_batches) * 100
    sys.stdout.write(f'\rProgress: |{bar}| {percent:.1f}% ({i + args.start_idx}/{total_batches})')
    sys.stdout.flush()
