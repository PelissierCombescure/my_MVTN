...
data_dir= Dataset_dir+'ModelNet40'#+'ScanObjectNN' # specifiy where did you put the data rel
...
## Opitmize code 1/3
train_loader = CustomDataLoader(dset_train, batch_size=batch_size, shuffle=True, drop_last=False, pin_memory=True)
test_loader = CustomDataLoader(dset_test, batch_size=batch_size, shuffle=False, drop_last=False, pin_memory=True)
...
print("Enregistrement dans : "+results_dir_current)
...
## Save the training information to a JSON file
...
for epoch in range(epochs):
    ...
    correct = 0.0
    # Use tqdm to create a progress bar
    with tqdm.tqdm(total=len(test_loader), desc=f'Testing Epoch {epoch + 1}/{epochs}', unit='batch') as pbar:
    ...
    if True:
        ...
        if test_accuracy > best_accuracy:
            ...
            torch.save(mvnetwork.state_dict(), results_dir_current+f'/best/mvnetwork_best_epoch_{epoch + 1}.pth')
            torch.save(mvtn.state_dict(), results_dir_current+f'/best/mvtn_best_epoch_{epoch + 1}.pth')
            torch.save(mvrenderer.state_dict(), results_dir_current+f'/best/mvrenderer_best_epoch_{epoch + 1}.pth')
        ...
        save_loss_acc(results_dir_current,train_losses, test_losses, train_accuracies, test_accuracies, plot_best=True, best_epoch=best_epoch) 

print("Training of "+str(results_dir_current)+" complete! Best epoch:", best_epoch)
